/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2023 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f7xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define W5x00_BRDY0_Pin GPIO_PIN_2
#define W5x00_BRDY0_GPIO_Port GPIOE
#define W5x00_BRDY1_Pin GPIO_PIN_3
#define W5x00_BRDY1_GPIO_Port GPIOE
#define W5x00_BRDY2_Pin GPIO_PIN_4
#define W5x00_BRDY2_GPIO_Port GPIOE
#define W5x00_BRDY3_Pin GPIO_PIN_5
#define W5x00_BRDY3_GPIO_Port GPIOE
#define LED_1_Pin GPIO_PIN_6
#define LED_1_GPIO_Port GPIOF
#define LED_2_Pin GPIO_PIN_7
#define LED_2_GPIO_Port GPIOF
#define MCO_Pin GPIO_PIN_0
#define MCO_GPIO_Port GPIOH
#define LED_3_Pin GPIO_PIN_2
#define LED_3_GPIO_Port GPIOC
#define LED_4_Pin GPIO_PIN_3
#define LED_4_GPIO_Port GPIOC
#define M2_IN1_Pin GPIO_PIN_5
#define M2_IN1_GPIO_Port GPIOA
#define M2_IN2_Pin GPIO_PIN_6
#define M2_IN2_GPIO_Port GPIOA
#define M2_IN3_Pin GPIO_PIN_7
#define M2_IN3_GPIO_Port GPIOA
#define BUILTINLED_Pin GPIO_PIN_0
#define BUILTINLED_GPIO_Port GPIOB
#define M1_IN4_Pin GPIO_PIN_11
#define M1_IN4_GPIO_Port GPIOF
#define M1_IN1_Pin GPIO_PIN_5
#define M1_IN1_GPIO_Port GPIOG
#define M1_IN2_Pin GPIO_PIN_8
#define M1_IN2_GPIO_Port GPIOG
#define W5x00_INT_Pin GPIO_PIN_8
#define W5x00_INT_GPIO_Port GPIOC
#define W5x00_RST_Pin GPIO_PIN_9
#define W5x00_RST_GPIO_Port GPIOC
#define TMS_Pin GPIO_PIN_13
#define TMS_GPIO_Port GPIOA
#define TCK_Pin GPIO_PIN_14
#define TCK_GPIO_Port GPIOA
#define STLK_RX_Pin GPIO_PIN_10
#define STLK_RX_GPIO_Port GPIOC
#define STLK_TX_Pin GPIO_PIN_11
#define STLK_TX_GPIO_Port GPIOC
#define LED_5_Pin GPIO_PIN_6
#define LED_5_GPIO_Port GPIOD
#define BUT_1_Pin GPIO_PIN_9
#define BUT_1_GPIO_Port GPIOG
#define M2_IN4_Pin GPIO_PIN_6
#define M2_IN4_GPIO_Port GPIOB
#define M1_IN3_Pin GPIO_PIN_0
#define M1_IN3_GPIO_Port GPIOE

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
